...
<form action="{{ route('admin.stock-types.store') }}" method="POST" 
    ...
    <div>
        ...
        <select name="display_type"
            ...
            <option value="dropdown" {{ old('display_type') == 'dropdown' ? 'selected' : '' }}>
                Dropdown (Best for many options like weights)
            </option>
            <option value="color-swatch" {{ old('display_type') == 'color-swatch' ? 'selected' : '' }}>
    ...
    <div class="bg-blue-600/10 border border-blue-600/30 rounded-lg p-4">
        <div class="flex gap-3">
            ...
            <div class="text-sm text-gray-300">
                ...
                <p>After creating this stock type, you'll be able to add specific options (like S, M, L or 1kg, 2kg, 3kg) to it.</p>
            </div>
        </div>
    </div>
